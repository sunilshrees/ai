...
const CreateComponent = (props: any) => {
  ...
  const validationSchema = Yup.object().shape({
    ...
    stories: Yup.string().required('This is required field'),
  ...
  useEffect(() => {
    if (isEdit) {
      ...
      setValue('stories', items?.data?.stories)
      setValue('position', items?.data?.position)
      setValue('image', items?.data?.image)

      setEditorValue(items?.data?.stories)
    }
  }, [isEdit])

  const handleEditorChange = (value: string) => {
    setEditorValue(value)
    setValue('stories', value, { shouldValidate: true })
  }
  ...
  async function onSubmit(data: any) {
    ...
    formData.append('stories', requestObject?.stories)
