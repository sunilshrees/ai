...
import CreateComponent from '@/container/products/product-demos/CreateComponent'
...
const Page = () => {
  ...
  const getProductDemoList = async () => {
    ...
    try {
      const response = await apiHandler.get(`${API_URLS.GET_PRODUCT_DEMO_API}`)
            ...
            {!state?.loading &&
              state?.data?.map((deta: any, index: number) => (
                <div key={index} className="">
                  <div className="border border-[#62B82F] rounded-[20px]  px-[1.25rem] py-[2rem] relative min-h-[250px]">
                    ...
                    {/* <div className="absolute bottom-4 left-0 text-[1.25rem] font-normal mb-[1rem] text-white z-20 px-[1.25rem]">
                      {deta?.heading ?? '-'}
                    </div> */}
                  </div>
                  <div className=" text-[1rem] py-1 font-semibold text-center">
                    {deta?.name ?? '-'}
                  </div>
